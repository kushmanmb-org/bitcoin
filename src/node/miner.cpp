...
{
    options.block_reserved_weight = std::clamp<size_t>(options.block_reserved_weight, MINIMUM_BLOCK_RESERVED_WEIGHT, MAX_HARD_FORK_BLOCK_WEIGHT);
    ...
    // Limit weight to between block_reserved_weight and MAX_HARD_FORK_BLOCK_WEIGHT for sanity:
    // block_reserved_weight can safely exceed -blockmaxweight, but the rest of the block template will be empty.
    // Note: the consensus-enforced weight limit is determined by nHardForkHeight in ContextualCheckBlock.
    options.nBlockMaxWeight = std::clamp<size_t>(options.nBlockMaxWeight, options.block_reserved_weight, MAX_HARD_FORK_BLOCK_WEIGHT);
    return options;
}
